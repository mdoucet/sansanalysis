        ...
        function processData() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                if (!format) {
                    ...
                    if (s.points.xerr) {
                        // lower / upper error
                        if (s.points.xerr.asymmetric) {
                            format.push({ exl: true, number: true, required: true, defaultValue: null });
                            format.push({ exu: true, number: true, required: true, defaultValue: null });
                        } else 
                            format.push({ ex: true, number: true, required: true, defaultValue: null });
                    }
                    if (s.points.yerr) {
                        // lower / upper error
                        if (s.points.yerr.asymmetric) {
                            format.push({ eyl: true, number: true, required: true, defaultValue: null });
                            format.push({ eyu: true, number: true, required: true, defaultValue: null });
                        } else 
                            format.push({ ey: true, number: true, required: true, defaultValue: null });
                    }
                        ...
                        for (m = 0; m < ps; ++m) {
                            ...
                            if (f) {
                                if (f.number && val != null) {
                                    ...
                                    if (isNaN(val))
                                        val = null;
                                    // fit errorbars inside canvas
                                    if (s.points.xerr && s.points.xerr.scaleAxis && (f.exl || f.ex))
                                        updateAxis(s.xaxis, p[0] - val, p[0] - val);
                                    if (s.points.xerr && s.points.xerr.scaleAxis && (f.exu || f.ex))
                                        updateAxis(s.xaxis, p[0] + val, p[0] + val);
                                    if (s.points.yerr && s.points.yerr.scaleAxis && (f.eyl || f.ey))
                                        updateAxis(s.yaxis, p[1] - val, p[1] - val);
                                    if (s.points.yerr && s.points.yerr.scaleAxis && (f.eyu || f.ey))
                                        updateAxis(s.yaxis, p[1] + val, p[1] + val);
                                }
        ...
        function drawSeriesPoints(series) {
            function plotPoints(datapoints, radius, fillStyle, offset, circumference, axisx, axisy) {
                ...
                for (var i = 0; i < points.length; i += ps) {
                    ...
                    var exl = exu = ex = eyl = eyu = ey = null;
                    var xerr = series.points.xerr, yerr = series.points.yerr;
                    if (xerr) {
                        if (xerr.asymmetric) {
                            exl = points[i + 2];
                            exu = points[i + 3];
                            if (yerr && yerr.asymmetric){
                                eyl = points[i + 4];
                                eyu = points[i + 5];
                            } else if (yerr) 
                                eyl = points[i + 4];
                    ...
                    var err = [xerr, yerr];
                    var err_limits = [exl, exu, eyl, eyu];

                    for (e = 0; e < err.length; e++){
                        var ax = [axisx, axisy][e];
		        if (err[e] && err[e].show) {
                            if (err[e] == xerr)
			        var upper = x + err_limits[e * err.length + 1], lower = x - err_limits[e * err.length];
                            else if (err[e] == yerr)
			        var upper = y + err_limits[e * err.length + 1], lower = y - err_limits[e * err.length];
			    ...
			    // error bar
                            if (err[e] == xerr) drawPath( [[upper, axisy.p2c(y)],[lower,axisy.p2c(y)]] );
                            else if (err[e] == yerr) drawPath( [[axisx.p2c(x),upper],[axisx.p2c(x),lower]] );

                            // upper cap
		            var halfWidth = Math.max(2,radius) + 2;
                            if (drawUpper) {
			        if (err[e].upperCap == '-'){
                                    if (err[e] == xerr) drawPath( [[upper,axisy.p2c(y) - halfWidth],[upper,axisy.p2c(y) + halfWidth]] );
                                    else if (err[e] == yerr) drawPath( [[axisx.p2c(x) - halfWidth,upper],[axisx.p2c(x) + halfWidth,upper]] );
			        } else if ($.isFunction(err[e].upperCap)){
                                    if (err[e] == xerr) xerr.upperCap(upper, axisy.p2c(y), halfWidth);
                                    else if (err[e] == yerr) yerr.upperCap(ctx, axisx.p2c(x), upper, halfWidth);
                                }
                            }
                            // lower cap
                            if (drawLower) {
			        if (err[e].lowerCap == '-'){
                                    if (err[e] == xerr) drawPath( [[lower,axisy.p2c(y) - halfWidth],[lower,axisy.p2c(y) + halfWidth]] );
                                    else if (err[e] == yerr) drawPath( [[axisx.p2c(x) - halfWidth,lower],[axisx.p2c(x) + halfWidth,lower]] );
			        } else if ($.isFunction(err[e].lowerCap)){
                                    if (err[e] == xerr) xerr.lowerCap(lower, axisy.p2c(y), halfWidth);
                                    else if (err[e] == yerr) yerr.lowerCap(ctx, axisx.p2c(x), lower, halfWidth);
                                }
                            }
                        }
                    }
